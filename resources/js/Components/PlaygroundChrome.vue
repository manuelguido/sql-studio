...
const { rawSQL, dbSchema, isDirty, save, cancel, loadFromText, loadDefaultTemplate, downloadAsFile, autoLayout } =
	usePlayground();
...
<template>
	<header
		class="flex h-11 shrink-0 items-center justify-between border-b hairline bg-[color:var(--color-chrome)] px-4"
	>
		<!-- Brand -->
		<div class="flex items-center gap-3">
			<div class="flex items-center gap-2">
				<span class="h-2 w-2 rounded-full bg-[color:var(--color-accent)]"></span>
				<span class="font-mono text-[12px] font-medium tracking-tight text-[color:var(--color-ink)]"
					>SQL Studio</span
				>
			</div>
			<span class="h-3 w-px bg-[color:var(--color-line-strong)]"></span>
			<span class="font-mono text-[10.5px] uppercase tracking-wider text-[color:var(--color-ink-3)]"
				>Schema Designer</span
			>
		</div>
		...
		<div class="flex items-center gap-1.5">
			<input ref="fileInput" type="file" accept=".sql,text/plain" class="hidden" @change="onFileChange" />
