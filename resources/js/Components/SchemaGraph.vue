...
const { dbSchema, uiState, selectTable, selectRelation, setPosition } = usePlayground();
...
<template>
	<div class="relative flex min-h-0 flex-1 overflow-hidden bg-[color:var(--color-canvas)]">
		<!-- Empty state -->
		<div v-if="!dbSchema.tables.length" class="flex flex-1 items-center justify-center">
			<div class="text-center">
				...
				<p class="mt-1 font-mono text-[11px] text-[color:var(--color-ink-4)]">
					...
					<span class="text-[color:var(--color-ink-2)]">Template</span>, or paste SQL into the editor.
				</p>
			</div>
		</div>

		<!-- Canvas -->
		<div v-else ref="canvasRef" class="relative flex-1 overflow-auto" @click="clearSelection">
			<!-- Inner canvas: grid is INSIDE so it pans with content -->
			<div class="canvas-inner relative" :style="{ width: CANVAS_W + 'px', height: CANVAS_H + 'px' }">
				<!-- SVG edges layer -->
				<svg
				...
				>
					...
					<g v-for="edge in edges" :key="edge.key">
						...
						<path
							...
							:stroke-width="
								edgeActive(edge) === 'selected' ? 1.75 : edgeActive(edge) === 'related' ? 1.5 : 1
							"
					...
					<ul>
						...
						>
							...
							<button
								...
								:class="
									connecting?.hover?.table === table.name && connecting?.hover?.column === col.name
										? 'anchor--hot'
										: ''
								"
